...
S3 是**被动触发**的 Skill，需要上层系统在以下时机调用：

### 主要触发方式：对话式确认（推荐）

当工务人员在审核对话中表达确认意图时自动触发：

| 触发关键词 | 示例 | 置信度 |
|-----------|------|--------|
| 直接确认 | "好了" / "可以了" / "没问题了" / "就这样吧" | 高 |
| 通过表述 | "通过" / "审核通过" / "确认" / "OK" | 高 |
| 交付指示 | "可以交付了" / "给服贸吧" / "最终版" | 高 |

**触发流程**：
```
工务说"好了" → Agent 检测确认意图 → 展示修订摘要 → 二次确认
                                              ↓
工务说"确认" → 调用 S3 学习 → 学习完成 → 交付服贸
```

详见：`S3-DIALOG-TRIGGER.md`（对话式触发机制设计文档）

### 备用触发方式

| 触发场景 | 判断依据 | 调用示例 |
|---------|---------|---------|
| 前端回调 | 用户点击确认按钮 | `final_report.status === "confirmed"` |
| 状态变更 | webhook 检测到状态变化 | 后端服务调用 S3 |
| 定时批处理 | 扫描 `status=confirmed` 且未学习 | 定时任务调用 S3 |
| 手动触发 | 工务主动点击"学习此修订" | API 调用 S3 |
...
{
  "artifacts": {
    "initial_report_json": { "status": "ok", ... },
    "final_report_json": { "status": "confirmed", ... },
    "edit_actions": [...],           // 可选：用户编辑记录
    "conversation_messages": [...]   // 可选：对话反馈
  }
}
```

- **`final_report.status`** = `"confirmed"` / `"final"` → 终稿已确认
- **`edit_actions`** 非空 → 存在明确的用户修改
- **`conversation_messages`** 非空 → 包含用户反馈意见

如果以上字段表明没有实质性修订，S3 仍会运行但 `quality_score` 会较低（<0.55），不会被存储为学习样本。
