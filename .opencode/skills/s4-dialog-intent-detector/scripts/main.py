...
用法:
  python3 main.py --action process_message --message "消息内容" --task-id "TASK-001" --pretty
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="对话意图检测器 - 主入口",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  # 处理用户消息
  python3 main.py --action process_message --message "风险等级调高一点" --task-id "TASK-001" --pretty
  ...
  python3 main.py --action process_message --json-input '{"message":"确认","task_id":"TASK-001"}' --pretty
    ...
    parser.add_argument("--org-id", default="default-org", help="组织 ID")
    parser.add_argument("--user-id", default="default-user", help="用户 ID")
    ...
    try:
        if args.action == "process_message":
            if not args.message:
                ...
                org_id = payload.get("org_id", args.org_id)
                user_id = payload.get("user_id", args.user_id)
            else:
