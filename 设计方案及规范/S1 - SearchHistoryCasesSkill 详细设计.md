...
| `记录内容_需求工具` | `tools_content` | JSONB | 工具清单，结构见第��节 4.5 |
...
RH-000961：内含船厂常规工作；工期���迟因素；试航二次登轮
```
...
三类物料字段结构**各不相同**，主要差异在于：耗材和专用工具有 `model`（型号）字段，需求工具有 `toolTypeNo`（工具类型编码）字段，且耗材/专用工具的 `toolTypeNo` 均为 `null`。入库时直接以 JSONB 存储，不展开为子表，应用层解析时需按字��分别处理。
